...
static pthread_mutex_t patientswait = PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t patientscondwait = PTHREAD_COND_INITIALIZER;
...
{
   mutex_lock(&patientswait);

   while (hd->all_patients[id].done != 1)
   {
      cond_wait(&patientscondwait, &patientswait);
   }
