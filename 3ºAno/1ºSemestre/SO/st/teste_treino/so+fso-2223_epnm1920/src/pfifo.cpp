...
{
   pfifo->fifoaccess = PTHREAD_MUTEX_INITIALIZER; // NOVOOOO
   ...
   mutex_lock(&pfifo->fifoaccess); // novo


   memset(pfifo->array, 0, sizeof(pfifo->array));
   pfifo->inp = pfifo->out = pfifo->cnt = 0;

   cond_broadcast(&pfifo->fifonotfull); // novo
   mutex_unlock(&pfifo->fifoaccess);  // novo

   // fazer bloqueio do acesso, a função faz o q tem a fazer dps broadcast e no fim unlock do acesso
}
...
void insert_pfifo(PriorityFIFO* pfifo, uint32_t id, uint32_t priority)
{
   ...
   mutex_lock(&pfifo->fifoaccess); // novo
   while (full_pfifo(pfifo)) // novo, ent esta a encher...
   {
      cond_wait(&pfifo->fifonotfull, &pfifo->fifoaccess);
   }
   ...
   cond_broadcast(&pfifo->fifonotempty); // novo
   mutex_unlock(&pfifo->fifoaccess); // novo
}
...
uint32_t retrieve_pfifo(PriorityFIFO* pfifo)
{
   ...
   while(empty_pfifo(pfifo)){ // enquanto estou a esvaziar, não está vazio
   ...
   // ta a tirar por isso faz broadcast a avisar q ja n esta cheio
