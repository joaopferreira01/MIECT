...
while TRANSMITTEDPACKETS<P               % Stopping criterium
    ...
    switch Event
        ...
        case DEPARTURE                     % If first event is a DEPARTURE
            % if all bits of the packet are transmitted correctly then
            % packet transmitted, if 1 bit fails -> packet counts as lost
            % probability of a packet having no errors: (1-B)^(PacketSize*8)
            % introduces randomness to packet generation, with and without
            % errors
            ...
            else    % If failure 1 bit fail, packet counts as lost
                LOSTPACKETS = LOSTPACKETS + 1;
            end
