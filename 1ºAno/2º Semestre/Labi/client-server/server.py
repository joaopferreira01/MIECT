...
def new_msg(client_sock):
    ...
    if pedido["op"] == "START":
        ...
        resposta = {"op": pedido["op"],
                    "status": False, "error": "Operação inválida!"}
...
def new_client(client_sock, request):
    ...
    for i in range(0, len(gamers.keys())):
        if request["client_id"] == list(gamers.keys())[i]:
            print("Já existe um cliente com o id " +
                  list(gamers.keys())[i] + ".\nEntrada proibida!\n")
            resposta = {"op": "START", "status": False,
                        "error": "Cliente existente!"}
            return resposta
    # obtain the secret number and number of attempts
    print("O cliente " + request["client_id"] +
    ...
    gamers.update({request["client_id"]: {"socket": client_sock, "cipher": None,
                                          "guess": secretNumber, "max_attempts": maxAttempts, "attempts": 0}})
...
def create_file():
    ...
    writer = csv.DictWriter(report, delimiter=",", fieldnames=[
                            "client_id", "secretNumber", "maxAttempts", "attemptsMaken", "result"])
...
def update_file(client_id, result):
    ...
    with open('report.csv', 'a') as fd:
        ...
        writer.writerow([client_id, client["guess"],
                         client["max_attempts"], client["attempts"], result])
...
def guess_client(client_sock, request):
    ...
    resposta = 0
    if count == 0:
        resposta = {"op": "GUESS", "status": False,
...
def stop_client(client_sock, request):
    ...
    for i in range(0, len(gamers.keys())):
        if id == list(gamers.keys())[i]:
            count += 1
    if count == 0:
        resposta = {"op": "QUIT", "status": False,
                    "error": "Cliente inexistente!"}
    resposta = None

    if request["attempts"] != gamers[id]["attempts"]:
        resposta = {"op": "QUIT", "status": False,
                    "error": "Número de jogadas inconsistente"}
        clean_client(client_sock)
        return resposta
    else:
        if request["number"] == gamers[id]["guess"]:
            if request["attempts"] > gamers[id]["max_attempts"]:
                ...
                resposta = {"op": "STOP", "status": True,
                            "guess": gamers[id]["guess"]}
